 ...
 * I actually really enjoyed this assignment.
 * It was very straightforward and was a
...
public class Heap<E> {
	...
	public void add(E v, double p) throws IllegalArgumentException {
		...
		if (map.get(v) != null) throw new IllegalArgumentException("Value is already in the heap");
		ensureSpace();
		b[size]= new VP(v, p);
		map.put(v, size);
		size++ ;
	...
	void bubbleDown(int k) {
		...
		while ((2 * k + 1 < size && compareTo(2 * k + 1, k) == 1) ||
			(2 * k + 2 < size && compareTo(2 * k + 2, k) == 1)) {
			if (2 * k + 2 >= size) {
				swap(2 * k + 1, k);
				k= 2 * k + 1;
			} else if (compareTo(2 * k + 1, 2 * k + 2) != 1) {
				swap(2 * k + 2, k);
				k= 2 * k + 2;
			} else {
				swap(2 * k + 1, k);
	...
	public void changePriority(E v, double p) {
		...
		if (map.get(v) == null) throw new IllegalArgumentException("Value is not in the heap");
		double x= b[map.get(v)].priority;
		b[map.get(v)].priority= p;
		if (compareTo(x, p) == 1) bubbleDown(map.get(v));
		else if (compareTo(x, p) == -1) bubbleUp(map.get(v));
	}
